...
\paragraph{Caveats.} Lifting requires \emph{real} polynomial exponents $B_j$ (a complex $\tilde{a}$ admits no real-valued domain indicator; \texttt{liftcomplex} fires otherwise). When the lifted Newton polytope is lower-dimensional (few distinct monomials), the automatic fan fails and an explicit \texttt{"FanData"} must be supplied. Sectors with \texttt{HasConstantTerm -> False} may have infinite true variance: the sample-based error bar can then underestimate the true error. See \texttt{SUMMARY.txt} \S8 and \texttt{SANDBOX/benchmark\_results.md} for benchmark tables.
